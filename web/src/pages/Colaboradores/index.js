...
const Colaboradores = () => {
  ...
  const { colaborador, colaboradores, servico, form, components, behavior } =
  ...
  useEffect(() => {
    ...
    const isEspecialidadesDefined = Array.isArray(colaborador.especialidades);
    const isServicoDefined = Array.isArray(servico);
        ...
        <Drawer.Body>
          ...
          <div className="row mt-3">
            ...
            <div className="col-12">
              ...
              />
              <Checkbox
                checked={isEspecialidadesDefined && isServicoDefined && colaborador.especialidades.length === servico.length}
                disabled={
                    (form.disabled && behavior === 'create') ||
                    (isEspecialidadesDefined && isServicoDefined && colaborador.especialidades.length === servico.length)
                }
                onChange={(v, checked) => {
                if (checked) {
                setColaborador(
                    'especialidades',
                    isServicoDefined ? servico.map((s) => s.value) : []
                );
                } else {
