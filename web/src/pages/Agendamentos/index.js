...
const Agendamentos = () => {
  ...
  const formatEventos = () => {
    console.log('Agendamentos:', agendamentos); // Adicione este log
    const listaEventos = agendamentos.map((agendamento) => ({
      resource: { agendamento },
      title: `${agendamento.servicoId?.titulo || 'Serviço Desconhecido'} - ${agendamento.clienteId?.nome || 'Cliente Desconhecido'} - ${agendamento.colaboradorId?.nome || 'Colaborador Desconhecido'}`,
      start: moment(agendamento.data).toDate(),
      end: moment(agendamento.data)
        .add(
          util.hourToMinutes(
            moment(agendamento.servicoId?.duracao || '00:00').format('HH:mm')
          ),
          'minutes'
        )
        .toDate(),
