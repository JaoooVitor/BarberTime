...
export function* addHorario() {
  try {
    ...
    const { data: res } = yield call(api.post, '/horario', {
      ...horario,
      barbeariaId: '66be77b0a029b87697d8740f',
    });
...
export function* allHorarios() {
  ...
  try {
    ...
    const { data: res } = yield call(
      api.get,
      `/horario/barbearia/66be77b0a029b87697d8740f`
    );
...
export function* allServicos() {
  ...
  try {
    ...
    const { data: res } = yield call(
      api.get,
      `/barbearia/servico/66be77b0a029b87697d8740f`
    );
    yield put(updateHorario({ form: { ...form, filtering: false } }));
    ...
    // Certifique-se de que a chave correta está sendo atualizada
